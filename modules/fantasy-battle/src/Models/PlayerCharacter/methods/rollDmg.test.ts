...
describe('PlayerCharacter.rollDmg()', () => {
  ...
  it('works', () => {
    ...
    expect(rollDmg.call(mockedPc, "Creation", 0)  .diceArgs).toEqual(expect.objectContaining({ dieAmmount:  1, dieMax:  2 }))
    expect(rollDmg.call(mockedPc, "Fortitude", -1).diceArgs).toEqual(expect.objectContaining({ dieAmmount:  2, dieMax: 10 }))
    expect(rollDmg.call(mockedPc, "Logic", +2)    .diceArgs).toEqual(expect.objectContaining({ dieAmmount: 10, dieMax: 10 }))
    expect(rollDmg.call(mockedPc, "Might", +5)    .diceArgs).toEqual(expect.objectContaining({ dieAmmount: 11, dieMax:  8 }))
  })
})
